...
st.caption("Yahoo Finance 데이터를 활용한 인터랙티브 주식 분석 도구")
...
ticker_input = st.sidebar.text_input(
    ...
    help="예: AAPL, TSLA, 005930.KS (한국 종목은 .KS/.KQ 접미사 사용)"
)
...
st.sidebar.markdown("---")
primary_ticker = st.sidebar.selectbox("상세 분석 종목 선택", tickers if tickers else ["AAPL"])

if not tickers:
    st.warning("사이드바에서 최소 하나의 종목 티커를 입력해주세요.")
    st.stop()
...
if primary_ticker not in data_dict:
    st.error(f"'{primary_ticker}' 데이터를 불러올 수 없습니다. 티커를 확인해주세요.")
    st.stop()
...
m1.metric(f"{primary_ticker} 현재가", f"${last_close:,.2f}", f"{change:+.2f} ({pct_change:+.2f}%)")
m2.metric("기간 최고가", f"${period_high:,.2f}")
m3.metric("기간 최저가", f"${period_low:,.2f}")
m4.metric("평균 거래량", f"{avg_volume:,.0f}")
if info.get("marketCap"):
    m5.metric("시가총액", f"${info['marketCap']/1e9:,.1f}B")
else:
    m5.metric("52주 최고", f"${info.get('fiftyTwoWeekHigh', 0):,.2f}" if info.get("fiftyTwoWeekHigh") else "N/A")
...
with tab1:
    ...
    if chart_type == "캔들스틱":
        fig.add_trace(go.Candlestick(
            ...
            low=df_main["Low"], close=df_main["Close"], name=primary_ticker,
            increasing_line_color="#26a69a", decreasing_line_color="#ef5350"
        ), row=1, col=1)
    else:
        fig.add_trace(go.Scatter(
            x=df_main.index, y=df_main["Close"], mode="lines",
            name=primary_ticker, line=dict(color="#2962ff", width=2)
        ), row=1, col=1)
    ...
    if show_volume:
        vol_colors = np.where(df_main["Close"] >= df_main["Open"], "#26a69a", "#ef5350")
...
with tab2:
    ...
    with col2:
        ...
        hist_colors = np.where(df_main["MACD_HIST"] >= 0, "#26a69a", "#ef5350")
...
with tab3:
    if len(data_dict) < 2:
        st.info("2개 이상의 종목을 입력하면 비교 차트가 표시됩니다.")
    else:
        st.subheader("정규화된 누적 수익률 비교 (시작일 = 100)")
        fig_compare = go.Figure()
        for t, df_t in data_dict.items():
            normalized = df_t["Close"] / df_t["Close"].iloc[0] * 100
            fig_compare.add_trace(go.Scatter(x=df_t.index, y=normalized, mode="lines", name=t))
        ...
        returns_df = pd.DataFrame({t: df_t["Close"].pct_change() for t, df_t in data_dict.items()}).dropna()
        ...
        for t, df_t in data_dict.items():
            ...
            summary_rows.append({
                "티커": t,
                "현재가": round(df_t["Close"].iloc[-1], 2),
                "기간 수익률(%)": round(total_return, 2),
                "연환산 변동성(%)": round(ret.std() * np.sqrt(252) * 100, 2),
                "최고가": round(df_t["High"].max(), 2),
                "최저가": round(df_t["Low"].min(), 2),
            })
        st.dataframe(pd.DataFrame(summary_rows).set_index("티커"), use_container_width=True)

# ── 탭 4: 데이터 테이블 ───────────────────────────
with tab4:
    st.subheader(f"{primary_ticker} 원본 데이터")
    ...
    st.download_button(
        label="📥 CSV 다운로드",
        data=csv,
        file_name=f"{primary_ticker}_data_{start_date}_{end_date}.csv",
        mime="text/csv",
    )

st.markdown("---")
st.caption("데이터 출처: Yahoo Finance (yfinance) · 투자 판단은 본인 책임입니다. 이 앱은 투자 자문을 제공하지 않습니다.")
